...
class MyDataset(Dataset):
    ...
    def __init__(self,
        ...
        train_test_threshold = (80 * len(groups)) // 100
        df_x_train = pd.concat(groups[:train_test_threshold])
        df_x_test = pd.concat(groups[train_test_threshold:])

        if split == "train":
            print(f"Train positives: {len(df_x_train[df_x_train['label']==1])}")
            print(f"Train negatives: {len(df_x_train[df_x_train['label']==0])}")

            train_data = [(img, label) for (img, label) in list(zip(imgss,labelsS)) if img in df_x_train['name'].tolist()]
            random.Random(1265).shuffle(train_data)
            imsh, labsh = zip(*train_data)
            self.imgs, self.labels = list(imsh), list(labsh)
        else:
            print(f"Test positives: {len(df_x_test[df_x_test['label']==1])}")
            print(f"Test negatives: {len(df_x_test[df_x_test['label']==0])}")
